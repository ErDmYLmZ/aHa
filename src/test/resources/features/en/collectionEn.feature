...
#    And the status code should be 200
  ...
  Scenario: Search for a non-existing item
    When user makes a search with "non-existing-keyword"
    Then the response should return an empty result
    And the status code should be 200

  Scenario: Search for an existing item
    When user makes a search with "young man"
    Then the response should contain search results related to "young man"
    And the status code should be 200